...
const userdata=[{"id":1,"first_name":"Hester","last_name":"Rentenbeck","email":"hrentenbeck0@marriott.com"},
...
const Search = () => {
    ...
    const deletehandler=(first_name)=>{
        const newdata=data.filter(user=>user.first_name!==first_name)
        setData(newdata)
    }
  return (
    <div>
        {
            data.map((user)=>{
                return(
                    ...
                    {user.first_name}{''}
                    {user.email}{''}
                    <button onClick={()=>deletehandler(user.first_name)}>delete</button>
                    </div>
